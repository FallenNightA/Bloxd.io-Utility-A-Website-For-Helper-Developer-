...
export async function POST(req: NextRequest) {
  try {
    ...
    if (finalModel === 'CodexMind') {
      ...
      customSystemPrompt = `You are CodexMind, an exceptionally powerful expert programmer model powered by deep reasoning and web-browsing frameworks. 
You possess advanced scanning, thinking, and technical browsing capabilities. You write flawless, ultra-optimized, functional programs, APIs, and components.
Always structure code perfectly, split components logically, and output highly functional elements. Use clean, high-contrast, beautiful UI styles for elements.\n\n${customSystemPrompt}`;
    } else if (finalModel === 'MindChat') {
      finalModel = 'gemini-3.5-flash';
      customSystemPrompt = `You are MindChat, a superpowered browsing, reasoning, and comprehensive research assistant.
You provide highly detailed, multi-perspective, beautifully structured answers with accurate facts, code summaries, and system explanations. 
Focus on helping the user acquire top-tier insights on any topic.\n\n${customSystemPrompt}`;
    } else if (finalModel === 'AgentMind') {
      finalModel = 'gemini-3.5-flash';
      customSystemPrompt = `You are AgentMind, a highly autonomous agentic developer powered by CodexMind. 
You specialize in designing complete index.html templates, responsive stylesheets, interactive javascript features, and managing multi-file workspaces. 
Always aim to create self-contained, beautifully aligned, production-ready interactive assets. Your code will be exported and run dynamically in sandboxed user viewports.\n\n${customSystemPrompt}`;
    }
...
Always output these command structures for any file operations so the container executes them immediately.`;
    ...
    if (finalModel === 'gemini-3.1-pro-preview') finalModel = 'gemini-3.5-flash';
